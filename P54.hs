...
parseText :: String -> (Hand, Hand)
parseText cs = (Hand (take 5 cards), Hand (drop 5 cards))
    where cards = map parseCard $ words cs

parseCard :: String -> Card
parseCard cs = Card value suit
    where suit = parseSuit $ head $ tail cs
          value = parseValue $ head cs
...
scoreHand (Hand cards) = scoreSortedHand $ sort cards

scoreSortedHand :: [Card] -> Score
scoreSortedHand cards =
    case sig of
      [(1,a),(1,b),(1,c),(1,d),(1,e)]
          | e-a == 4 && sameSuit cards -> StraightFlush e
          | e-a == 4 -> Straight e
          | sameSuit cards -> Flush [e,d,c,b,a]
          | otherwise -> Highcard [e,d,c,b,a]
      [(1,a),(1,b),(1,c),(2,d)] -> OnePair d [c,b,a]
      [(1,a),(2,b),(2,c)] -> TwoPair c b [a]
      [(1,a),(1,b),(3,c)] -> ThreeKind c [b,a]
      [(2,a),(3,b)] -> FullHouse b a
      [(1,a),(4,b)] -> FourKind b [a]
    where
      sig = valueSig cards

valueSig :: [Card] -> [(Int, Value)]
valueSig cards = sort $ valueSig1 values
    where values = map value cards
          valueSig1 [] = []
          valueSig1 (v:vs) = valueSig3 vs 1 v
          valueSig3 [] count val = [(count, val)]
          valueSig3 (v:vs) count val
              | v == val = valueSig3 vs (count+1) val
              | otherwise = (count, val) : valueSig1 (v:vs)

sameSuit :: [Card] -> Bool
sameSuit [c] = True
sameSuit (c:d:cs)
    | suit c == suit d = sameSuit (d:cs)
    | otherwise = False
...
main :: IO ()
main = putStrLn $ show $ length $ filter id $ map scoringFunction input

-- Outputs true if player 1 wins
scoringFunction :: String -> Bool
scoringFunction = (uncurry compareHands) . parseText

compareHands :: Hand -> Hand -> Bool
compareHands left right
    | leftScore < rightScore = False
    | leftScore == rightScore = error "Equal hands"
    | otherwise = True
    where leftScore = scoreHand left
